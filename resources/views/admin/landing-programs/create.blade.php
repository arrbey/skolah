...
<div class="p-6 max-w-4xl">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-2">Tambah Program Baru</h1>
    </div>

    <form action="{{ route('admin.landing-programs.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 space-y-6">
        @csrf
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Judul Program</label>
                <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Contoh: E-learning">
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Subtitle / Tagline</label>
                <input type="text" name="subtitle" value="{{ old('subtitle') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Contoh: Pelajari ratusan skill sekali bayar">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
            <div>
                ...
                <input type="text" name="button_text" value="{{ old('button_text', 'Lihat Detail') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Tombol Link</label>
                <input type="text" name="button_link" value="{{ old('button_link', '#') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
            <x-image-upload 
                ...
                info="600 x 600 px (PNG Transparan direkomendasikan)" 
        ...
        <div class="pt-6 border-t">
            <button type="submit" class="px-8 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg transition">Simpan Program</button>
        </div>
    </form>
</div>
@endsection

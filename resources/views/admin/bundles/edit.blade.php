...
@section('title', 'Edit Bundle: ' . $bundle->title)
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" rows="5" 
                              class="tinymce w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $bundle->description) }}</textarea>
                </div>
                        ...
                        @foreach($courses as $course)
                            <label class="relative flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl cursor-pointer hover:border-blue-300 hover:bg-blue-50 transition-all group">
                                <input type="checkbox" name="course_ids[]" value="{{ $course->id }}" 
                                       ...
                                       {{ in_array($course->id, old('course_ids', $selectedCourses)) ? 'checked' : '' }}>
        ...
        <div class="space-y-6">
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                ...
                <div>
                    ...
                    <select name="instructor_id" class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($instructors as $inst)
                            <option value="{{ $inst->id }}" {{ old('instructor_id', $bundle->instructor_id) == $inst->id ? 'selected' : '' }}>{{ $inst->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $bundle->price) }}" required
                ...
                <div>
                    ...
                    <input type="number" name="discount_price" value="{{ old('discount_price', $bundle->discount_price) }}"
                           class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
                ...
                <x-image-upload name="thumbnail" label="Thumbnail Bundle" info="Rasio 16:9 disarankan" :value="$bundle->thumbnail_url" />
            </div>

...
<div class="grid lg:grid-cols-3 gap-6">
                    ...
                    <div class="relative">
                        ...
                        <div class="bg-gray-50 rounded-lg p-3 border border-gray-100">
                            ...
                            <div class="mt-1 space-y-0.5 text-xs text-gray-500">
                                @if($history->courier)
                                    <p>Kurir: <span class="font-semibold">{{ $history->courier_label }}</span></p>
                                @endif
                                @if($history->tracking_number)
                                    <p>Resi: <span class="font-mono font-semibold text-gray-700">{{ $history->tracking_number }}</span></p>
                                @endif
                                @if($history->note)
                                    <p class="italic text-gray-500">"{{ $history->note }}"</p>
                                @endif
    ...
    <div class="space-y-5">
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{ status: '{{ $bookOrder->shipping_status }}' }">
            <div class="px-5 py-3 border-b border-gray-100 bg-gray-50">
                <h3 class="text-sm font-semibold text-gray-700">🔄 Update Status Pengiriman</h3>
            </div>
            <div class="p-5">
                <form action="{{ route('admin.book-orders.update-status', $bookOrder) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <select name="shipping_status" x-model="status"
                            ...
                            <option value="pending"    {{ $bookOrder->shipping_status === 'pending'    ? 'selected' : '' }}>🕐 Menunggu Pengiriman</option>
                            <option value="processing" {{ $bookOrder->shipping_status === 'processing' ? 'selected' : '' }}>📦 Diproses</option>
                            <option value="shipped"    {{ $bookOrder->shipping_status === 'shipped'    ? 'selected' : '' }}>🚚 Dalam Pengiriman</option>
                            <option value="cancelled"  {{ $bookOrder->shipping_status === 'cancelled'  ? 'selected' : '' }}>❌ Dibatalkan</option>
                        </select>
                    </div>

                    {{-- Kurir & Resi (tampil jika shipped) --}}
                    ...
                    <div>
                        ...
                        <textarea name="note" rows="2" placeholder="Catatan untuk dikirim ke user..."
        ...
        {{-- Form: Upload Foto Bukti Terima (tampil jika shipped) --}}
        ...
        <div class="bg-white rounded-xl border border-green-200 overflow-hidden">
            <div class="px-5 py-3 border-b border-green-100 bg-green-50">
                <h3 class="text-sm font-semibold text-green-700">✅ Konfirmasi Paket Diterima</h3>
            </div>
            <div class="p-5">
                <form action="{{ route('admin.book-orders.confirm-delivery', $bookOrder) }}" method="POST"
                      enctype="multipart/form-data" class="space-y-4">
                    @csrf

                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">
                            Foto Bukti Terima <span class="text-red-500">*</span>
                        </label>
        ...
        <div class="bg-gray-50 rounded-xl border border-gray-200 p-5 text-center text-sm text-gray-500">
            @if($bookOrder->shipping_status === 'delivered')
                ✅ Pesanan ini sudah selesai terkirim.
            @else
